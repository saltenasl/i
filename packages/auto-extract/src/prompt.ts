export const buildSystemPromptV2 = (): string => {
  return [
    ...
    'Schema:',
    '{"title":string,"noteType":string,"summary":string,"language":string,"date":string|null,"sentiment":"positive"|"negative"|"neutral"|"varied","emotions":[{"emotion":string,"intensity":1|2|3|4|5}],"entities":[{"id":string,"name":string,"type":"person"|"org"|"tool"|"place"|"concept"|"event","evidenceText"?:string,"context"?:string,"confidence":number}],"facts":[{"id":string,"ownerEntityId":string,"perspective":"self"|"other"|"uncertain","subjectEntityId"?:string,"predicate":string,"objectEntityId"?:string,"objectText"?:string,"evidenceText":string,"confidence":number}],"relations":[{"fromEntityId":string,"toEntityId":string,"type":string,"evidenceText"?:string,"confidence":number}],"todos":[{"id":string,"description":string,"assigneeEntityId"?:string,"evidenceText":string,"confidence":number}],"groups":[{"name":string,"entityIds":string[],"factIds":string[]}]}',
    'Rules:',
    '- Grounding is strict: all names and evidenceText MUST be exact, unmodified substrings from the input text.',
    '- Use minimal canonical entity names (example: "Egle", not "Egle was driving"). Entity "name" MUST be an exact substring extracted from the text. DO NOT summarize or normalize entity names. The ONLY exception is "ent_self" which is always "I".',
    '- Put phrase-level role/context in evidenceText and optional context.',
    '- evidenceText MUST be an exact quote from the text. Do not modify or summarize the text.',
    ...
    '- Actions/emotions should be represented in fact/todo evidenceText, not inside entity names.',
